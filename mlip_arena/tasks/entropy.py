...
def get_entropy_from_path(subset_path, dataset_path, dataset_desc_out_path, k=32, cutoff=5.0, h=0.015):
    """
...
def get_trajectory_entropy(trajectory_dir, start_idx, end_idx, step, dataset_desc_path, k=32, cutoff=5.0, h=0.015):
    """
...
def run_simulations(model_names, structures, out_dir):
    ...
    for model_name in model_names:
        ...
        for i, atoms in enumerate(structures):
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Running {model_name} on structure number {i}")
            ...
            if len(supercell_atoms) > max_atoms:
                print(f"Skipping structure {i} because it has too many atoms ({len(supercell_atoms)} > {max_atoms})")
...
def run(
    ...
    dH = get_trajectory_entropy(trajectory_dir, start_idx, end_idx, step, dataset_desc_path, k=k, cutoff=cutoff, h=h)
