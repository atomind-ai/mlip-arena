...
def compare_forces(
    ...
    if np.any(valid_forces):
        norms_product = (
            np.linalg.norm(rotated_original_forces[valid_forces], axis=1) * 
...
def save_molecule_results(aggregate_results: dict, idx_list: np.ndarray, save_path: str | Path) -> None:
    ...
    np.save(save_path.with_name(f"{save_path.stem}_cosine_similarities.npy"), cosine_similarities)
    np.save(save_path.with_name(f"{save_path.stem}_idx_list.npy"), idx_list)

@task(name="Equivariance testing",
...
def run(
    ...
    for atom_idx, atoms in enumerate(atoms_list):
        ...
        molecule_results = {
            ...
            "avg_cosine_similarity_by_molecule": float(np.mean([results_by_angle[angle]["mean_cosine_similarity"] for angle in rotation_angles])),
            "avg_mae_by_molecule": float(np.mean([results_by_angle[angle]["avg_mae"] for angle in rotation_angles])),
            "overall_equivariant_ratio": float(np.mean([
    ...
    aggregate_results = {
        ...
        "average_equivariant_ratio": float(np.mean([
            result["overall_equivariant_ratio"] for result in all_results
        ])),
        ...
        "average_mae_by_angle": {
            angle: float(np.mean(diffs))
            for angle, diffs in cross_molecule_mae.items()
        },
    ...
    if save_path:
        ...
        np.save(str(save_path.with_name(f"{save_path.stem}_molecule_results.npy")), all_results)
